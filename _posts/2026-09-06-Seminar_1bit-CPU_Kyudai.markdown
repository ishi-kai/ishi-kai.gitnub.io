---
layout: post
title:  "ISHI会ハンズオン in 九州大学大学院システム情報科学府附属価値創造型半導体人材育成センターを開催しました"
date:   2026-09-06 00:00:00 +0900
categories: Seminar shuttle OpenSUSI-TR10
---
## ISHI会ハンズオン in 九州大学大学院システム情報科学府附属価値創造型半導体人材育成センターを開催しました
「[九州大学大学院システム情報科学府附属価値創造型半導体人材育成センターが主催する2026年度実習シリーズでISHI会のハンズオンが採用されました。](https://ishi-kai.org/seminar/2026/08/30/Seminar_KyushuUniv_seminar.html)」でご案内し通り、1日目に去年の参加者を対象として[1bit-CPUハンズオン](https://github.com/ishi-kai/openmpw-transistor-level-examples/tree/main/OpenSUSI-TR10/1bit-CPU) を開催し、2日目に初めての人向けの[インバータハンズオン](https://github.com/ishi-kai/OpenEDA-PDK_SetupScript/blob/main/docs/inverter_OpenSUSI-TR10.pdf)を開催しました。  

### 1日目：[1bit-CPUハンズオン](https://github.com/ishi-kai/openmpw-transistor-level-examples/tree/main/OpenSUSI-TR10/1bit-CPU)
参加者は3名と少なめになりました。やはり、去年の学生さんはすでに卒業されていたりなど、2年目限定となると参加者が厳しいですね。  
ただ、今回は教員の方の参加となったため、来年は期待出来そうな気はします。  

- ![1日目の風景](/assets/images/Seminar/kyushuUniv2026_day1_01.jpg)

### 2日目：[インバータハンズオン](https://github.com/ishi-kai/OpenEDA-PDK_SetupScript/blob/main/docs/inverter_OpenSUSI-TR10.pdf)
今回は、約20名くらいの参加者となり大盛況となりました。  
見事に、全員、最後まで完走することができたので、来年の測定会は楽しみな結果となりそうです。  
完走の内訳は、下記のような感じです。今年は、AIサポートの力が大きかったですね。  

- 1/3の6名くらいの方は、サクサクと進めて時間が余る
- 1/6の3名くらいの方が、大半の部分をサポート要員の力を借りて、なんとか終了
- 1/3の6名くらいの方が、AIの力を借りつつも自力で、ちょうどギリギリでついてきた
- 2名の方が、進行よりは遅れ気味ではあったが、AIの力を借りつつも自力で、なんとか終了
- 最後の1名は、環境構築で大苦戦して、時間の大半をセットアップに費やし、ギリギリで終了

- ![2日目の風景1](/assets/images/Seminar/kyushuUniv2026_day2_01.jpg)
- ![2日目の風景2](/assets/images/Seminar/kyushuUniv2026_day2_02.jpg)


## 資料
- [1bit-CPU](https://github.com/ishi-kai/openmpw-transistor-level-examples/tree/main/OpenSUSI-TR10/1bit-CPU)
    - HDLやロジック回路（デジタル回路）がどのようにGDSに変換されているのかの学習が目的のハンズオンです。
    - 「 [ISHI会版OpenMPW TR10-2でICHIKENさんが体験された1bit-CPUの解説動画が公開されました。](https://ishi-kai.org/openmpw/shuttle/tr10/2026/06/08/shuttle_ISHI-Kai_OpenMPW-TR10-2_ichiken.html)」
