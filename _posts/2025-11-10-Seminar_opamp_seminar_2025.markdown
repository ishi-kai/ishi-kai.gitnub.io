...
そこで、そのspice monkeyを自動でやらせるためのソフトウェアの使い方などについて解説する講座です。  
...
- テーマ：多目的値,多パラメータ最適化ソフト(自作)の回路への適用
    - 第1回
        - 開催日
            - 11/xx（xx）22時～
        - 内容
            - 例題として用意ある26パラメータ3段オペアンプの例題(LTSpice)を私の環境を画面共有して実演その1
    - 第2回
        - 開催日
            - 11/xx（xx）22時～
        - 内容
            - 例題として用意ある26パラメータ3段オペアンプの例題(LTSpice)を私の環境を画面共有して実演その2
    - 第3回
        - 開催日
            - 11/xx（xx）22時～（定例アナログ系もくもく会）
        - 内容
            - 御興味のある方のPCへのセットアップサポート 
                - Ubuntuを使います。WSLは上手くいかなさそうなのでVMwareかVirtulBox上。
    - 第4回
        - 開催日
            - 11/xx（xx）22時～（定例アナログ系もくもく会）
        - 内容
            - まだ私が実施していないngspiceへの適用(pythonでi/f合わせを少しする位の筈)をどなたかのPC環境を画面共有して実施
- 参考書
